...
import { ProjectVisitorsDto, CreateProjectVisitorRequestDto, CreateProjectVisitorResponseDto, ProjectVisitorParamDto, ProjectVisitorsCountResponseDto } from 'src/shared-dtos/src/recruiter_project.dto';
...
export class ProjectVisitorsController {
  ...
  async createProjectVisitor(
    ...
    this.logger.log(`Attempting to create a project visitor for user ID: ${user_id}`);
    ...
    try {
      ...
      this.logger.error(`Error creating project visitor for user ID: ${user_id} - ${error.message}`, error.stack);
  ...
  async getProjectVisitors(
    ...
    try {
      ...
      this.logger.error(`Error fetching visitors for project ID: ${projectId} - ${error.message}`, error.stack);
      return { error: true, message: error.message };
    }
  }
}
