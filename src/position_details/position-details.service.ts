...
export class PositionDetailsService {
  ...
  async createOrUpdatePositionDetails(
    data: PositionDetailsRequestDto,
  ): Promise<{ error: boolean; message: string }> {
    const logger = new Logger('PositionDetailsService');

    try {
      const { position_id: positionId, positionData, companyData, ...restData } = data;

      logger.log(
        `Received request to create or update position details for position ID: ${positionId}`,
      );
      ...
      logger.log(`Searching for position with ID: ${positionId}`);
      let position: Position = await this.positionRepository.findOne({
        where: { id: parseInt(positionId) },
      });

      if (!position) {
        logger.warn(`Position not found with ID: ${positionId}`);
        return { error: true, message: 'Position not found.' };
      }
      logger.log(`Position found with ID: ${positionId}`);
      ...
      logger.log(`Checking for existing position details for position ID: ${positionId}`);
      let positionDetails: PositionDetails = await this.positionDetailsRepository.findOne({
        where: { position_id: positionId },
      });

      if (!positionDetails) {
        logger.log(
          `No existing position details found. Creating new details for position ID: ${positionId}`,
        );
        ...
        logger.log(
          `Existing position details found. Updating details for position ID: ${positionId}`,
        );
      ...
      logger.log(`Position details saved for position ID: ${positionId}`);

      const positionUpdate = { details: null, company: null };
      ...
      position = this.positionRepository.merge(
        position,
        positionData ? positionData : positionUpdate,
      );
      await this.positionRepository.save(position);

      logger.log(`Position with ID: ${positionId} updated successfully.`);
  ...
  async getPositionDetails(positionId: string): Promise<PositionDetailsResponseDto> {
    try {
      this.logger.log(`Fetching position details for position ID: ${positionId}`);

      const positionDetails: PositionDetailsDto = await this.positionDetailsRepository.findOne({
        where: { position_id: positionId },
      });

      if (!positionDetails) {
        this.logger.warn(`No position details found for position ID: ${positionId}`);
        return { error: true, message: 'Position details not found.' };
      }

      this.logger.log(`Successfully retrieved position details for position ID: ${positionId}`);
      return {
        error: false,
      ...
      this.logger.error(
        `Error fetching position details for position ID: ${positionId} - ${error.message}`,
        error.stack,
      );
      return { error: true, message: 'Position details not found.' };
    }
