...
import { PositionDetailParamDto, PositionDetailsRequestDto, PositionDetailsResponseDto } from 'src/shared-dtos/src/position_detail.dto';
...
export class PositionDetailsController {
  ...
  async createOrUpdatePositionDetails(
    ...
    this.logger.log(`Received request to create or update position details for position ID: ${data.position_id}`);
  ...
  async getPositionDetails(
    ...
    this.logger.log(`Received request to fetch position details for position ID: ${position_id}`);

    try {
      const result = await this.positionDetailsService.getPositionDetails(position_id);
      return result;
    } catch (error) {
      this.logger.error(`Error while fetching position details for position ID: ${position_id} - ${error.message}`);
