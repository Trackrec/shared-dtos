...
export class PositionDetailsController {
  ...
  async createOrUpdatePositionDetails(@Body() data:PositionDetailsRequestDto): Promise<{ error: boolean, message: string}> {
    try {
      if (!data.position_id) {
  ...
  @Get('/:position_id')
  async getPositionDetails(@Param('position_id') position_id: string): Promise<PositionDetailsResponseDto> {
    try {
