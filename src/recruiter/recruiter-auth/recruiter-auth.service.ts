...
export class RecruiterAuthService {
  ...
  async registerUser(
    ...
    try {
      ...
      this.logger.error(
        `Error occurred while registering user with email ${email}: ${error.message}`,
      );
  ...
  async findOrCreate(userDto: {
    ...
    const { email, displayName, accessToken, username, loginMethod } = userDto;
    ...
    try {
      ...
      if (email) {
        ...
        this.logger.log(
          `Checking if user with username ${username} exists with role 'Admin' or 'User'`,
        );
      ...
      if (user) {
        ...
        if (user.login_method == loginMethod) {
          this.logger.log(
            `User with ${email ? 'email' : 'username'} ${email || username} already exists and login method matches`,
          );
          return { error: false, user };
        } else {
          this.logger.warn(
            `User with ${email ? 'email' : 'username'} ${email || username} already exists, but login method does not match`,
          );
          return {
            error: true,
  ...
  async updateCompanyUser(
    ...
    try {
      this.logger.log(
        `Starting user update. Target User ID: ${id}, Performed by User ID: ${userId}`,
      );

      // Ensure all fields are provided
      if (!email.trim() || !fullName.trim() || !role.trim()) {
        this.logger.warn(
          `Validation failed: Missing required fields. Email: ${email}, Full Name: ${fullName}, Role: ${role}`,
        );
        return { error: true, message: 'All fields are required.' };
      }

      // Check if the current user is an admin
      this.logger.log(`Verifying admin access for User ID: ${userId}`);
      ...
      if (!checkAdmin) {
        this.logger.warn(`Access denied. User ID ${userId} is not an admin.`);
      ...
      const existingAssociation: RecruiterCompanyUser =
      ...
      if (email !== user.email) {
        ...
        const existingUser: UserDto = await this.userRepository.findOne({
          where: { email, role: In(['User', 'Admin']) },
        });

        if (existingUser) {
          this.logger.warn(
            `Email update conflict: Another user with email ${email} already exists.`,
          );
  ...
  async updateUser(
    ...
    try {
      ...
      this.logger.log(
        `Checking if the username '${updateUserPayload.public_profile_username}' is already taken.`,
      );
      ...
      if (existingUser && existingUser.id !== id) {
        this.logger.warn(
          `Public profile username '${updateUserPayload.public_profile_username}' is already in use by User ID: ${existingUser.id}`,
        );
        return {
          error: true,
      ...
      this.logger.log(
        `Updating user details for User ID: ${id}. Payload: ${JSON.stringify(updateUserPayload)}`,
      );
  ...
  async updatepreference(
    ...
    try {
      ...
      this.logger.debug(
        `Preference update payload for User ID ${id}: ${JSON.stringify(updateUserPreferencePayload)}`,
      );
      ...
      this.logger.error(
        `Failed to update preferences for User ID: ${id}. Error: ${error.message}`,
        error.stack,
      );
  ...
  async getMe(userId: number): Promise<{
    error: boolean;
    message?: string;
    user?: { user: UserDto; company: RecruiterCompanyDto };
  }> {
    ...
    try {
      ...
      this.logger.error(
        `Error fetching user details for User ID: ${userId}. Error: ${error.message}`,
        error.stack,
      );
  ...
  async createUser(
    ...
    this.logger.log(
      `Admin user (ID: ${userId}) initiated the creation of a new user with email: ${email}`,
    );
    ...
    try {
      ...
      if (!checkAdmin) {
        this.logger.warn(
          `User (ID: ${userId}) attempted to create a user without admin privileges.`,
        );
        return { error: true, message: 'You are not an admin user.' };
      }
      ...
      if (!recruiterCompanyUser) {
        this.logger.warn(
          `Admin user (ID: ${userId}) is not associated with any recruiter company.`,
        );
      ...
      const assignCompanyToNewUser: RecruiterCompanyUserDto =
      ...
      this.logger.log(
        `Assigned new user (ID: ${newUser.id}) to company (ID: ${recruiterCompanyUser.company.id}).`,
      );
      ...
      const messageData = {
        ...
        html: `
            ...
            <p>Hey ${fullName}, please use the following credentials to log in:</p>
            <p>Email: ${email}</p>
  ...
  async changePassword(
    ...
    this.logger.log(`User (ID: ${userId}) initiated a password change request.`);

    try {
      // Check if required fields are provided
      if (!currentPassword || !newPassword) {
        this.logger.warn(
          `User (ID: ${userId}) did not provide all required fields for password change.`,
        );
        return {
          error: true,
      ...
      if (!user) {
        this.logger.warn(`Password change failed: User (ID: ${userId}) does not exist.`);
        return {
          error: true,
      ...
      const oldPasswordValid: boolean = await bcrypt.compare(currentPassword, user.password);

      if (!oldPasswordValid) {
        this.logger.warn(`User (ID: ${userId}) provided an incorrect current password.`);
        return {
          error: true,
      ...
      this.logger.log(`User (ID: ${userId}) successfully changed their password.`);
      ...
      this.logger.error(`Error changing password for user (ID: ${userId}): ${e.message}`, e.stack);
      return {
        error: true,
  ...
  async deleteUser(userId: number): Promise<{ error: boolean; message: string }> {
    ...
    try {
      ...
      if (companyUser) {
        this.logger.log(
          `User (ID: ${userId}) is associated with company (ID: ${companyUser.company.id}).`,
        );

        // If the user is the owner of the company
        if (user.companyCreated && user.companyCreated.id === companyUser.company.id) {
          this.logger.warn(
            `Deletion blocked: User (ID: ${userId}) is the owner of company (ID: ${companyUser.company.id}).`,
          );
        ...
        this.logger.log(
          `User (ID: ${userId})'s association with company (ID: ${companyUser.company.id}) deleted.`,
        );
      }
      ...
      if (userProjects.length > 0) {
        this.logger.log(
          `User (ID: ${userId}) has ${userProjects.length} associated projects. Reassigning projects.`,
        );
  ...
  async sendResetEmail(email: string): Promise<{ error: boolean; message: string }> {
    ...
    try {
      ...
      this.logger.error(
        `Error while processing password reset for email ${email}: ${e.message}`,
        e.stack,
      );
  ...
  async verifyToken(token: string): Promise<VerifyTokenResponse> {
    ...
    try {
      ...
      this.logger.log(
        `Password reset token verified for user (ID: ${user.id}, Email: ${user.email}).`,
      );
      return { error: false, message: 'Token verified', token };
    } catch (error) {
      this.logger.error(
        `Error during token verification for token: ${token} - ${error.message}`,
        error.stack,
      );
      return { error: true, message: 'Password reset token is invalid or has expired.' };
    }
