...
export class RecruiterCompanyService {
  ...
  async createCompany(company_name: string, userId: number, buffer: Buffer, imageType: string): Promise<CreateRecruiterCompanyDto> {
    this.logger.log('info', `Starting company creation for user ${userId} with company name: ${company_name}`);
    ...
    if (existingAssociation) {
      this.logger.log('warn', `User ${userId} already has an associated company. Company name: ${company_name}`);
      return {
        error: false,
    ...
    this.logger.log('info', `Image for company ${company_name} uploaded successfully. Image type: ${imageType}`);
    ...
    this.logger.log('info', `Company ${company_name} created and saved successfully for user ${userId}`);
  
    const user: UserDto = await this.userRepository.findOne({ where: { id: userId } });
  
    if (!user || user.role !== 'Admin') {
      this.logger.log('error', `Invalid recruiter: User ${userId} is not an admin or does not exist.`);
    ...
    this.logger.log('info', `User ${userId} successfully associated with company ${company_name}`);
  ...
  async updateCompany(
    ...
    if (company_name) {
      this.logger.log('info', `Updating company name for company ID ${companyId} to ${company_name}`);
    ...
    if (buffer && imageType) {
      ...
      this.logger.log('info', `Uploading new image for company ID ${companyId} with image type ${imageType}`);
      let storedImage: string = await this.uploadService.uploadNewImage(buffer, 'recruiter_company_images', imageType);
  ...
  async findAllUsersInCompany(userId: number): Promise<UsersInCompanyResponseDto> {
    try {
      ...
      this.logger.log(`Company ID ${companyId} found for user ID ${userId}. Fetching users associated with the company.`);
