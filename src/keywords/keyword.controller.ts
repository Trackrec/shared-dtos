...
export class KeywordsController {
  ...
  async createOrUpdateKeywords(
    ...
    this.logger.log(`Request to create or update keywords for user ID: ${userId} with keywords: ${JSON.stringify(keywords)}`);

    try {
      const result = await this.keywordsService.createOrUpdateKeywords(userId, keywords);
      return result;
    } catch (error) {
      this.logger.error(`Error updating keywords for user ID: ${userId} - ${error.message}`, error.stack);
  ...
  async getKeywords(
    ...
    try {
      ...
      this.logger.error(`Error fetching keywords for user ID: ${userId} - ${error.message}`, error.stack);
